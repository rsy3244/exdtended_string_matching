...
\begin{frame}{オプション文字：$\ShiftAnd$への適用}
	\begin{block}{改良した$\ShiftAnd$アルゴリズム}
		...
		\begin{itemize}
			...
			\item $I$：直前がオプション文字でなく、\\オプション文字がある状態を$1$としたビットマスク
			\item $F$：直後がオプション文字でなく、\\オプション文字がある状態を$1$としたビットマスク
		\end{itemize}
	\end{block}
	\vspace{-1em}
	...
	\begin{columns}
		\begin{column}{.33\textwidth}
			\begin{tikzpicture}[state/.style={circle, draw, minimum size=.7cm}, node distance=0cm]
				...
				\foreach \x [count=\i from 1, remember=\i as \prev (initially 0)] in {0,0,1,0,0,1,0} {
		...
		\begin{column}{.66\textwidth}
			\only<1>{
				\begin{tikzpicture}[state/.style={circle, draw, minimum size=.7cm}, myLabel/.style={minimum width={width("WWWWW")}}, node distance=0cm]
					...
					\foreach \x [count=\i from 1, remember=\i as \prev (initially 0)] in {0,0,0,0,0,1,0} {
			...
			\only<2>{
				\begin{tikzpicture}[state/.style={circle, draw, minimum size=.7cm}, node distance=0cm]
					...
					\foreach \x [count=\i from 1, remember=\i as \prev (initially 0)] in {0,0,0,0,1,0,0} {
						\node(dp\i) [rectangle, draw, right =of dp\prev] {$\x$};
					}
				\end{tikzpicture}
			}
			\only<3>{
				\begin{tikzpicture}[state/.style={circle, draw, minimum size=.7cm}, myLabel/.style={minimum width={width("(F-(DW I))W W F")}},node distance=0cm]
					...
					\foreach \x [count=\i from 1, remember=\i as \prev (initially 0)] in {1,1,1,1,0,1,1} {
						\node(d\i) [rectangle, draw, right =of d\prev] {$\x$};
					}
					\node(is0) [rectangle, draw] at (0,-.7) {$0$};
					...
					\foreach \x [count=\i from 1, remember=\i as \prev (initially 0)] in {1,1,0,1,1,0,1} {
						\node(dp\i) [rectangle, draw, right =of dp\prev] {$\x$};
					}
				\end{tikzpicture}
			}
